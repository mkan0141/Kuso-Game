...
void init_enemy(Enemy &enemy, Player player,
...
void init_charactor(Player &player, Enemy &enemy,
                    int (&maze)[MAZE_SIZE][MAZE_SIZE]) {
    player = Player(1, 1);
    enemy = Enemy(9, 9);
...
void destroy_all(int (&maze)[MAZE_SIZE][MAZE_SIZE], Player *player,
                 Enemy *enemy) {
    free(player);
    free(enemy);
}
...
void game_main(char *name) {
    ...
    Enemy *enemy;
    enemy = (Enemy *)malloc(sizeof(Enemy) * 10);
    Vector2 *vec;
    vec = (Vector2 *)malloc(sizeof(Vector2) * 10);
    int *enemy_walk;
    enemy_walk = (int *)malloc(sizeof(int) * 10);
    int ch;
        ...
        if (ch != -1) {
            if (available_move(*player, Vector2(dx[ch], dy[ch]), maze)) {
                for (int i = 0; i < enemy_cnt; i++)
                    vec[i] = ai(enemy[i], *player, maze, enemy_walk[i]);
                for (int i = 0; i < enemy_cnt; i++) {
                ...
                move_player(*player,
                            Vector2(player->x + dx[ch], player->y + dy[ch]),
                            maze);
                for (int i = 0; i < enemy_cnt; i++) {
                    if (is_game_over(enemy[i], *player)) {
                        gameover = true;
