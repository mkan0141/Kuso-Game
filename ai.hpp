...
int bfs(int x, int y, Player player, int _maze[MAZE_SIZE][MAZE_SIZE]) {
    ...
    for (int i = 0; i < MAZE_SIZE; i++)
        for (int j = 0; j < MAZE_SIZE; j++) maze[i][j] = _maze[i][j];

    std::queue<std::pair<Vector2, int>> que;
    que.push(std::make_pair(Vector2(x, y), 0));

    while (que.size()) {
        std::pair<Vector2, int> tmp = que.front();
        que.pop();
...
Vector2 ai(Enemy &enemy, Player player, int (&maze)[MAZE_SIZE][MAZE_SIZE],
           int &walk) {
    walk++;
    if (walk < 20)
        return random_ai(enemy, maze);
    else
        return bfs_ai(enemy, player, maze);
}
